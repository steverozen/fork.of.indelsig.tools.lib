...
#' @export
prepare_indels <- function(indels, sampleID, genome.v, verbose = TRUE) {
  ...
  if (is.character(indels) == TRUE) {
    ...
    stop(
      '[error prepare_indels] Provide either a path to a vcf file or a tab file with chr, position, REF, ALT'
    )
  }
  if (verbose) {
    message("[info prepare_indels] Indel-specific preps")
  }
  df <- indelsig.tools.lib:::prepfunc(
    indels,
    sampleID = sampleID,
    genome.v = genome.v
  )
  ...
  for (i in seq_along(pyr_bases)) {
    ...
    df[
      cur_base,
      'slice5_pyr'
    ] <- as.character(Biostrings::reverseComplement(Biostrings::DNAStringSet(df$slice3[
      cur_base
    ])))
    df[
      cur_base,
      'slice3_pyr'
    ] <- as.character(Biostrings::reverseComplement(Biostrings::DNAStringSet(df$slice5[
      cur_base
    ])))
  }
  return(df)
...
segment_indels <- function(df, verbose = TRUE) {
  ...
  if (
    any(c('unit', 'prime3_rep', 'internal_rep', 'spacer') %in% colnames(df))
  ) {
    df <- df[,
      !(colnames(df) %in%
        c(
